...
# ════════════════════════════════════════════════════════════════════════
# AlphaForge 新增：币安合约「另类数据」因子（crypto_alt）
# ------------------------------------------------------------------------
# 每个 compute 都是 (raw_dict) -> Tensor[N, T]。当所需的另类列不存在于 raw_dict
# 时（例如 MT5/外汇数据、或只有 OHLCV 的 1m 数据），一律返回全零（中性），从而：
#   - 老数据/老流程完全不受影响（这些因子恒 0，会被评估器当常数列剪掉）；
#   - 在币安含 oi/funding/多空比/主动买卖比/CVD 的数据上，这些因子才「活」起来，
#     进入公式搜索空间。
# 归一化统一复用因果滚动 _robust_norm，量纲与既有价量因子可比。
# ════════════════════════════════════════════════════════════════════════

_EPS_ALT = 1e-9


def _alt_zero(raw: dict) -> torch.Tensor:
    return torch.zeros_like(raw["close"])


def _alt_get(raw: dict, key: str):
    v = raw.get(key)
    return v if isinstance(v, torch.Tensor) else None


def _alt_safe_log(x: torch.Tensor) -> torch.Tensor:
    return torch.log(torch.clamp(x, min=_EPS_ALT))


def _alt_pct1(x: torch.Tensor) -> torch.Tensor:
    """逐 bar 百分比变化，首列置 0（因果）。"""
    prev = torch.cat([x[:, :1], x[:, :-1]], dim=1)
    return (x - prev) / (prev.abs() + _EPS_ALT)


def _c_funding_z(raw):
    f = _alt_get(raw, "funding_rate")
    return _alt_zero(raw) if f is None else _fe._robust_norm(f)


def _c_funding_mom(raw):
    f = _alt_get(raw, "funding_rate")
    return _alt_zero(raw) if f is None else _fe._robust_norm(f - _fe._ma(f, 48))


def _c_oi_chg(raw):
    oi = _alt_get(raw, "oi")
    return _alt_zero(raw) if oi is None else _fe._robust_norm(_alt_pct1(oi))


def _c_oi_price_div(raw):
    """价/仓背离：价涨且持仓增(+, 趋势健康)；价跌且持仓增(-, 空头加仓)。"""
    oi = _alt_get(raw, "oi")
    if oi is None:
        return _alt_zero(raw)
    r = _alt_pct1(raw["close"])
    d_oi = _fe._robust_norm(_alt_pct1(oi))
    return _fe._clean(torch.sign(r) * d_oi)


def _c_taker_imbalance(raw):
    tbs = _alt_get(raw, "taker_bs")
    return _alt_zero(raw) if tbs is None else _fe._robust_norm(_alt_safe_log(tbs))


def _c_taker_buy_frac(raw):
    tbv = _alt_get(raw, "taker_buy_vol")
    vol = raw.get("volume")
    if tbv is None or vol is None:
        return _alt_zero(raw)
    frac = tbv / (vol.abs() + _EPS_ALT)
    return _fe._robust_norm(frac - 0.5)


def _c_topls_pos(raw):
    x = _alt_get(raw, "topls_pos")
    return _alt_zero(raw) if x is None else _fe._robust_norm(_alt_safe_log(x))


def _c_topls_acc(raw):
    x = _alt_get(raw, "topls_acc")
    return _alt_zero(raw) if x is None else _fe._robust_norm(_alt_safe_log(x))


def _c_global_acc(raw):
    x = _alt_get(raw, "global_acc")
    return _alt_zero(raw) if x is None else _fe._robust_norm(_alt_safe_log(x))


def _c_smart_dumb_div(raw):
    """大户账户多空比 vs 全局账户多空比 的背离（聪明钱 vs 散户）。"""
    top = _alt_get(raw, "topls_acc")
    glob = _alt_get(raw, "global_acc")
    if top is None or glob is None:
        return _alt_zero(raw)
    return _fe._robust_norm(_alt_safe_log(top) - _alt_safe_log(glob))


def _c_quote_vol_surge(raw):
    qv = _alt_get(raw, "quote_vol")
    if qv is None:
        return _alt_zero(raw)
    ma = _fe._ma(qv, 96)
    return _fe._robust_norm(_alt_safe_log((qv + _EPS_ALT) / (ma + _EPS_ALT)))


def _c_cvd_imbalance(raw):
    cvd = _alt_get(raw, "cvd_15s")
    if cvd is None:
        return _alt_zero(raw)
    vol = raw.get("volume")
    denom = (vol.abs() + _EPS_ALT) if isinstance(vol, torch.Tensor) else 1.0
    return _fe._robust_norm(cvd / denom)


_ALT_FEATURE_DEFS = [
    ("FUNDING_Z",        "crypto_alt", _c_funding_z),
    ("FUNDING_MOM",      "crypto_alt", _c_funding_mom),
    ("OI_CHG",           "crypto_alt", _c_oi_chg),
    ("OI_PRICE_DIV",     "crypto_alt", _c_oi_price_div),
    ("TAKER_IMBALANCE",  "crypto_alt", _c_taker_imbalance),
    ("TAKER_BUY_FRAC",   "crypto_alt", _c_taker_buy_frac),
    ("TOPLS_POS",        "crypto_alt", _c_topls_pos),
    ("TOPLS_ACC",        "crypto_alt", _c_topls_acc),
    ("GLOBAL_ACC",       "crypto_alt", _c_global_acc),
    ("SMART_DUMB_DIV",   "crypto_alt", _c_smart_dumb_div),
    ("QUOTE_VOL_SURGE",  "crypto_alt", _c_quote_vol_surge),
    ("CVD_IMBALANCE",    "crypto_alt", _c_cvd_imbalance),
]

for _name, _category, _compute in _ALT_FEATURE_DEFS:
    if _ACTIVE_FEATURES is not None and _name not in _ACTIVE_FEATURES:
        continue
    FEATURE_REGISTRY.register_feature(
        FeatureSpec(name=_name, category=_category, compute=_compute)
