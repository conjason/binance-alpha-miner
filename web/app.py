"""FastAPI application for BinanceAlphaMiner training UI."""
...
def api_ai_analyze_training(req: AnalyzeTrainingRequest):
    ...
    # 安全 fork：移除上游按 key 前缀自动切到本地 token 通道（openclaw/openclaw_wb）的逻辑，
    # provider 只从请求或设置里取，Key 一律由用户显式提供。
...
def api_realtime_feishu_test(req: FeishuTestRequest) -> dict[str, Any]:
    ...
    ok, msg = send_text(
        "✅ BinanceAlphaMiner 飞书通知测试：配置正常。信号方向转折时会推送提醒。",
        webhook_url=url,
        secret=secret or "",
    )
    if not ok:
