...
function resolveAiFromKey(raw) {
  // 安全 fork：不再有本地 token 自动通道（openclaw / openclaw_wb），
  // 一律走 DeepSeek，Key 由用户显式填写。
...
function updateAiChannelHint() {
  ...
  hint.textContent = row?.hint || "当前：DeepSeek（deepseek-v4-flash · https://api.deepseek.com）· 需自填 API Key。";
}
